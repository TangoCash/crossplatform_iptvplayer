...
import sys
from initSysPaths import E2root
...
print "E2 root folder:",E2root
if os.access('/var/tuxbox/config', os.W_OK): # to keep settings in neutrino config folder
    configFolder = '/var/tuxbox'
else: #if not accessible we use subfolder where emulator installed
    configFolder = E2root
print "E2 config folder:%s/config" % configFolder
    
defaultPaths = {
		SCOPE_TRANSPONDERDATA: ("%s/" % configFolder, PATH_DONTCREATE),
		SCOPE_SYSETC: ("%s/" % configFolder, PATH_DONTCREATE),
		SCOPE_FONTS: ("/usr/local/share/fonts/", PATH_DONTCREATE),
		SCOPE_CONFIG: ("%s/config/" % configFolder, PATH_CREATE),
		SCOPE_PLUGINS: ("%s/Plugins/" % E2root, PATH_CREATE),

		SCOPE_LANGUAGE: ("%s/locale/" % E2root, PATH_DONTCREATE),
		SCOPE_GOSLANGUAGE: ("%s/locale/" % E2root, PATH_DONTCREATE),

		SCOPE_SKIN: ("%s/enigma2skin/" % E2root, PATH_DONTCREATE),
		SCOPE_SKIN_IMAGE: ("%s/enigma2skin/" % E2root, PATH_DONTCREATE),
		SCOPE_HDD: ("/hdd/movie/", PATH_DONTCREATE),
		SCOPE_MEDIA: ("/media/", PATH_DONTCREATE),
		SCOPE_PLAYLIST: ("%s/config/playlist/" % configFolder, PATH_CREATE),

		SCOPE_USERETC: ("", PATH_DONTCREATE), # user home directory

		SCOPE_METADIR: ("/usr/local/share/meta", PATH_CREATE),
	}
