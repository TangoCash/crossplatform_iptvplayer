...
class HardwareInfo:
    ...
    def __init__(self):
        ...
        # Version
        try:
            self.device_version = open("/proc/stb/info/version").read().strip() #first for e2 like tuners
        except:
            try:
                self.device_version = open("/proc/version_signature").read().strip() #second for linuxes
                self.device_name = "LINUX"
            except:
                try:
                    self.device_version = open("/proc/version_signature").read().strip() #third, silly android
