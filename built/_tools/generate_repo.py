...
class Generator:
    ...
    def __init__( self ):
        ...
        self.output_path='./kodi_repo/' #self.config.get('locations', 'output_path')
    ...
    def _generate_repo_files ( self ):
        ...
        if os.path.isfile(addonid + os.path.sep + "addon.xml"):
            os.remove(addonid + os.path.sep + "addon.xml")
        
        print "Create repository addon.xml"
        
        with open (self.tools_path + os.path.sep + "template.xml", "r") as template:
            template_xml=template.read().decode("utf-8")

        repo_xml = template_xml
        #.format(
        #    addonid= addonid,
        #    name=name,
        #    version=version,
