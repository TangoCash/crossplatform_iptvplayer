...
class OtakuFR(CBaseHostClass):
    ...
    def __init__(self):
        CBaseHostClass.__init__(self, {'history':'otakufr.com', 'cookie':'otakufr.cookie', 'cookie_type':'MozillaCookieJar'})
    ...
    def getFavouriteData(self, cItem):
        printDBG('OtakuFR.getFavouriteData')
        return json.dumps(cItem) 
        
    def getLinksForFavourite(self, fav_data):
        printDBG('OtakuFR.getLinksForFavourite')
    ...
    def setInitListFromFavouriteItem(self, fav_data):
        printDBG('OtakuFR.setInitListFromFavouriteItem')
        if self.MAIN_URL == None:
            self.selectDomain()
        try:
