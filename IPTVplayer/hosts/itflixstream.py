...
###################################################
def gettytul():
    return 'https://itflix.stream/'

class ItflixStream(CBaseHostClass):
    
    def __init__(self):
        CBaseHostClass.__init__(self, {'history':'itflix.stream', 'cookie':'itflix.stream.cookie', 'cookie_type':'MozillaCookieJar'})
        self.USER_AGENT = 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:40.0) Gecko/20100101 Firefox/40.0'
        self.MAIN_URL = 'https://itflix.stream/'
        self.DEFAULT_ICON_URL = 'https://itflix.stream/wp-content/uploads/2018/02/itflix.png'
    ...
    def listMainMenu(self, cItem, nextCategory1, nextCategory2):
        ...
        data = self.cm.ph.getDataBeetwenNodes(data, ('<ul', '>', 'main_header'), ('</ul', '>'), False)[1]
    ...
    def listSubItems(self, cItem):
        printDBG("ItflixStream.listSubItems")
    ...
    def listSearchResult(self, cItem, searchPattern, searchType):
        printDBG("ItflixStream.listSearchResult cItem[%s], searchPattern[%s] searchType[%s]" % (cItem, searchPattern, searchType))
    ...
    def getLinksForVideo(self, cItem):
        printDBG("ItflixStream.getLinksForVideo [%s]" % cItem)
    ...
    def getVideoLinks(self, baseUrl):
        printDBG("ItflixStream.getVideoLinks [%s]" % baseUrl)
    ...
    def getArticleContent(self, cItem):
        printDBG("ItflixStream.getArticleContent [%s]" % cItem)
        ...
        tmp = self.cleanHtmlStr(self.cm.ph.getDataBeetwenNodes(data, ('<b', '</b>', 'Classificazione TMDb'), ('</div', '>'), False)[1])
...
class IPTVHost(CHostBase):
    ...
    def __init__(self):
        CHostBase.__init__(self, ItflixStream(), True, [])
