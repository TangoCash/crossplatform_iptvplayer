...
# LOCAL import
###################################################
from Plugins.Extensions.IPTVPlayer.dToolsSet.iptvtools import iptv_system, printDBG
    ...
    def __call__(self, *args, **kwargs):
        if self.proxyQueue:
            if self.proxyQueue.mainThreadName != threading.currentThread().getName():
                ...
                printDBG("raise BaseException(Delegate cannot be used in MainThread!)")
...
class MainSessionWrapper(object):
    ...
    def waitForFinishOpen(self, *args, **kwargs):
        ...
        if tmpRet and tmpRet[0] == MainSessionWrapper.WAIT_RET: 
            ...
            printDBG("raise BaseException(DelegateToMainThread Error)")
